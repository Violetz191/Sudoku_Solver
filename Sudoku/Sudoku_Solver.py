...
# Return columns's lists of cells
...
# Same for blocks
#This list comprehension is unreadable, bu quite cool!
...
# The 10 strategies to solve the sudoku
########################################
    ...
    for group in all_houses:
        for cell in group:
            if len(sudoku[cell]) == 1:
                for cell2 in group:
                    if sudoku[cell] in sudoku[cell2] and cell != cell:
...
# 3. Intersection
# Include: pointing pairs, box line reduction
###############################################
...
# 7. Nice Chains
# aka X-cycles, nice loops
#############################

def get_soft_link_from_group(s, n, group):
    found =[]
    for cell in group:
        if n in s[cell]:
            found.append(cell)
...
def get_all_soft_links(s, n):
    soft_links = []
    for group in all_houses:
        new_links = get_soft_link_from_group(s, n, group)
        if new_links != []:
            ...
            if len(s[i, j])>1 and cell_in_chain((i, j), a) and not cell_in_chain((i, j), b):  # 2+ numbers in cell, from one chain, but not from the other
                ...
                for n in s[(i,j)]: # Go through the numbers in the cell
                    if n != in_cell: # Except for the one from the chain
                        for house in all_houses: # Look at all the houses
...
def empty_by_colors(s, all_a, all_b):
    count = 0
    for (i, j) in range2(9, 9):
                        ...
                        if (i, j) in house:
                            for cell in house:
                                if cell_in_chain(cell, a) and get_n_cell_in_chain(cell, a) ==n:
                                    found.append(get_n_cell_in_chain(cell, s))
...
def brute_force(s, verbose):
    ...
    if len(solution)>0:
        if verbose:
            print ("Backtracking tookL", time.time(), "second, with", iter_counter, "attempt made")
        return solution
...
# Control how solver goes through methods:
# False - go back to previous method if the next one yeld result
# True - try all methods one by one and then go back
    ...
    #print_sudoku(puzzle)
    if verbose:
        print ("Solved with logic: number of complete cells", solved, "/81. Candicates to remove:", to_remove)
